p.vectorp <- function (data, design = NULL, Q = 0.05, MT.adjust = "BH", min.obs = 3, counts=FALSE, family=NULL, theta=10, epsilon=0.00001) 
{
...
#----------------------------------------------------------------------
		p.vector  <-  foreach(chunk=ichunk(1:g,chunkSize=round(g/15),mode='integer'), .combine='c') %dopar% {
			for (i in chunk){
				y <- as.numeric(dat[i, ])
					div <- c(1:round(g/100)) * 100
					if (is.element(i, div)) {
						print(paste(c("A worker fitting gene", i, "to", tail(chunk, n=1)), collapse = " "))
					}
