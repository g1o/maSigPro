		...
		min.obs = data$min.obs, alfa = data$Q, nvar.correction = FALSE, family=gaussian(), epsilon=0.00001 ) 
{
	...
	cl <- makeCluster(15,outfile="")
		...
		lista <- foreach (chunk=ichunk(2:(g + 1) ,chunkSize=round(g/15),mode='integer'), .export=c("stepback","stepfor","two.ways.stepback","two.ways.stepfor","position"), .combine='comb', .multicombine=TRUE, .init=list(list(), list(),list(),list() )) %dopar% { 
			for (i in chunk){
