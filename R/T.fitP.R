T.fitp <- function (data, design = data$dis, step.method = "backward", 
		min.obs = data$min.obs, alfa = data$Q, nvar.correction = FALSE, family=gaussian(), epsilon=0.00001 , ncores=15)
{
	comb <- function(x, ...) {
		lapply(seq_along(x),
	...
	registerDoParallel(cl)
		lista <- foreach (chunk=ichunk(2:(g + 1) ,chunkSize=g/ncore,mode='integer'), .export=c("stepback","stepfor","two.ways.stepback","two.ways.stepfor","position"), .combine='comb', .multicombine=TRUE, .init=list(list(), list(),list(),list() )) %dopar% { 
			print(paste(c("A worker is fitting gene", chunk[1]-1, "to", tail(chunk, n=1)), collapse = " ")) 
			for (i in chunk){
...
#			sol <- rbind(as.numeric(c(p.value, bondad, p.valor)))
#				sig.profiles <- rbind(y)
#				t.score <- rbind(t)
#				coefficients <- rbind(coeff)
			...
			print(paste(c("Finished the chunk from  gene", chunk[1]-1, "to", tail(chunk, n=1)), collapse = " ")) 
				list(sol,coefficients,t.score,sig.profiles)
		}
	stopCluster(cl)

	sol		<-do.call(rbind, lista[[1]])
	coefficients	<-do.call(rbind, lista[[2]])
	t.score		<-do.call(rbind, lista[[3]])
	sig.profiles	<-do.call(rbind, lista[[4]])
